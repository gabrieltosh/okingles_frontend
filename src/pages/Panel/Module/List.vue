...
<div>
    <div class="q-px-md q-pt-md q-gutter-sm">
        <q-breadcrumbs>
            ...
            <q-breadcrumbs-el label="Modulos" icon="eva-map-outline" to="/start/pick-quasar-flavour" />
...
export default {
  ...
  methods: {
    ...
    handleEdit (props) {
      this.$router.push({ name: 'module.edit', params: { data: props.row } })
    },
    handleDelete (props) {
      this.$q.dialog({
        title: 'Confirmar',
        message: '¿ Eliminar el registro Modulo ?',
        ok: {
          color: 'primary',
          label: 'Aceptar',
          rounded: true,
        ...
        var url = '/panel/module/delete/' + props.row.id
        this.$axios.delete(url).then(response => {
          this.$q.notify({
            color: 'positive',
            message: '¡ Modulo eliminada exitosamente !',
            timeout: 500,
