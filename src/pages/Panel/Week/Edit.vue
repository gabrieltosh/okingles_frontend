    ...
    <div>
        <div class="q-px-md q-pt-md q-gutter-sm">
            <q-breadcrumbs>
                ...
                <q-breadcrumbs-el label="Semanas" icon="eva-map-outline" to="/start/pick-quasar-flavour" />
                <q-breadcrumbs-el label="Editar" to="/vue-components/breadcrumbs" />
            </q-breadcrumbs>
        </div>
        <div class="q-pa-md">
            <q-card class="my-card">
                <q-form @submit="handleUpdateWeek" @reset="handleFormReset">
                    <q-card-section>
                        <div class="text-subtitle1 text-primary">Actualizar Week</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-6 q-pa-md">
                                <q-input disable rounded dense label="Fecha de Inicio" v-model="data.start" mask="date" lazy-rules :rules="[ val => !!val || 'El campo es requerido']">
                                    <template v-slot:prepend>
                                        <q-icon name="eva-edit-2-outline" />
                                    </template>
                             ...
                             <div class="col-xs-12 col-sm-6 col-md-6 q-pa-md">
                                 <q-input disable rounded dense label="Fecha de Finalización" v-model="data.end" mask="date" lazy-rules :rules="[ val => !!val || 'El campo es requerido']">
                                    <template v-slot:prepend>
                                        <q-icon name="eva-edit-2-outline" />
                                    </template>
                                    <template v-slot:append>
                        ...
                        <div align="center">
                            <q-btn :to="{name:'week.list',params: {data:this.$route.params.branch}}" label="Cancelar" type="reset" color="primary" flat />
...
export default {
  ...
  methods: {
    handleUpdateWeek () {
      var url = '/panel/week/update/week'
      this.$axios.post(url, this.data).then(response => {
        this.$router.push({ name: 'week.list', params: { data: this.$route.params.branch } })
        this.$q.notify({
          color: 'positive',
    ...
    handleFormReset () {
      this.data.branch_office_id = null
