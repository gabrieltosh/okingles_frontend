...
<div>
    <div class="q-px-md q-pt-md q-gutter-sm">
        <q-breadcrumbs>
            ...
            <q-breadcrumbs-el label="Sucursales" icon="eva-map-outline" to="/start/pick-quasar-flavour" />
    ...
    <div class="q-pa-md">
        <q-table :filter="search" title="Sucursales" :data="data" :columns="columns" row-key="name">
            <template v-slot:top>
                ...
                <q-space />
                <q-btn :to="{name:'branch.create'}" size="sm" rounded icon="eva-plus" color="primary" label="Crear" />
                <q-space />
...
export default {
  ...
  methods: {
    ...
    handleEdit (props) {
      this.$router.push({ name: 'branch.edit', params: { data: props.row } })
    },
    handleDelete (props) {
      this.$q.dialog({
        title: 'Confirmar',
        ...
        var url = '/panel/branchoffice/delete/' + props.row.id
